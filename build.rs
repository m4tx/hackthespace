...
use imageproc::image::{ImageReader, RgbaImage};
...
fn main() -> anyhow::Result<()> {
    ...
    println!("cargo:rerun-if-changed=assets/images/source.png");
    println!("cargo:rerun-if-changed=assets/fonts/Roboto/Roboto-Black.ttf");

    Ok(())
}

fn build_css() {
    const SCSS_FILES: [&str; 4] = ["main", "finish", "sky", "keypad"];
    ...
    for scss_file in SCSS_FILES {
        ...
        let mut css = grass::from_path(&scss_path, &grass_options).expect("failed to compile SCSS");
...
fn generate_image() -> anyhow::Result<()> {
    let mut img = ImageReader::open("assets/images/source.png")?.decode()?;
    let font = std::fs::read("assets/fonts/Roboto/Roboto-Black.ttf")?;
